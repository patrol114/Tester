...
def initialize_and_train_model(train_json_path, val_json_path, model_name="bert-base-uncased", num_labels=2, epochs=2):
    ...
    # Przykładowe dane
    train_tensors = []  # Tutaj umieść tensor z danymi treningowymi
    val_tensors = []    # Tutaj umieść tensor z danymi walidacyjnymi

    # Tworzenie Dataset i DataLoader
    train_dataset = TensorDataset(train_tensors)  # Tutaj umieść odpowiednie tensory
    val_dataset = TensorDataset(val_tensors)      # Tutaj umieść odpowiednie tensory
    ...
    # Validation loop
    if len(val_data) > 0 and len(val_labels) > 0:
        model.eval()
        val_loss = 0
        with torch.no_grad():
...
if __name__ == '__main__':
    with torch.no_grad():
        ...
        train_data = []  # Your actual train data here
        val_data = []  # Your actual validation data here
        train_labels = []  # Your actual train labels here
        val_labels = []  # Your actual validation labels here

        # Convert lists to tensors
        train_tensors = torch.tensor(train_data)  # Conversion to tensor
        val_tensors = torch.tensor(val_data)  # Conversion to tensor
        train_labels = torch.tensor(train_labels)  # Conversion to tensor
        val_labels = torch.tensor(val_labels)  # Conversion to tensor

        # Now you can safely use TensorDataset
        train_dataset = TensorDataset(train_tensors, train_labels)
        val_dataset = TensorDataset(val_tensors, val_labels)

        train_json_path = "cleaned_train.json"
        val_json_path = "cleaned_valid.json"
        bert_model = initialize_and_train_model(train_json_path, val_json_path)
        ...
        train_data_chunk = load_data_chunk('/', 'cleaned_train.json', start=0, chunk_size=10)
        ...
        test_data_chunk = load_data_chunk('/', 'cleaned_test.json', start=0, chunk_size=10)
