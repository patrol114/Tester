...
def train_classification_model(train_data, train_labels):
    if isinstance(train_data, np.ndarray):
        if train_data.size == 0 or np.all(train_data == 0):
            logging.error("Training data is empty or contains only zeros. Cannot train the model.")
            return None
    else:  # Assuming it's a list of lists
        if not train_data or all(all(not d for d in sublist) for sublist in train_data):
