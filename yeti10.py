...
def initialize_and_train_model(train_json_path, val_json_path, model_name="bert-base-uncased", num_labels=2, epochs=2):
    ...
    # Wczytaj dane treningowe i walidacyjne z plików JSON
    ...
    if len(train_data) > 0 and len(train_labels) > 0:
        ...
        print("Training data or labels are empty. Skipping DataLoader creation.")

    if len(val_data) > 0 and len(val_labels) > 0:
        val_dataset = TensorDataset(val_data, val_labels)
        val_loader = DataLoader(val_dataset, batch_size=32, shuffle=False)
    else:
        print("Validation data or labels are empty. Skipping DataLoader creation.")
...
if __name__ == '__main__':
    with torch.no_grad():
        ...
        bert_model = initialize_and_train_model(train_data, train_labels, val_data, val_labels, num_labels=2, epochs=2)
