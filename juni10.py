...
def display_results_and_ask_questions(generated_text_result):
    ...
    while True:
        ...
        table.add_row([generated_text[:550] + '...'])
        print(table)
...
if __name__ == '__main__':
    with torch.no_grad():
        ...
        print("Czyszczenie danych do tokenizacji...")
        cleaned_data = [tokenize_and_clean(item.get('text', '')) for item in data]
        labels = [item.get('label', 0) for item in data]

        # Display sample data
        print("Pierwsze 5 wyrazow, oczyszczone dane i etykiety do treningu:")
        print(cleaned_data[:5])
        print(labels[:5])

        # Train and evaluate classification model
        print("Trenowanie modelu klasyfikacji...")
