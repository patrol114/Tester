...
if __name__ == '__main__':
    with torch.no_grad():
        ...
        train_data = load_json_data(train_json_path)  # Zakładam, że funkcja `load_json_data` zwraca listę
        val_data = load_json_data(val_json_path)  # Zakładam, że funkcja `load_json_data` zwraca listę

        # Przekształcenie list na tensory
        train_tensors = torch.tensor(train_data)  # Konwersja na tensor
        val_tensors = torch.tensor(val_data)  # Konwersja na tensor

        # Teraz możesz bezpiecznie użyć TensorDataset
